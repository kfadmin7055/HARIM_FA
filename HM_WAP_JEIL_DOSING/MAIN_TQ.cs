...
{
    public partial class MAIN_TQ : DevExpress.XtraEditors.XtraForm
    {
        /// <summary>        /// 실행 경로y
        /// </summary>
        ...
        private void SetMainPlcConn()
        {
            ...
            if (connOk)
            {
                ...
                plcThread = new Thread(PLCWorker);
        ...
        private void PLCWorker()
        {
            ...
            clsLog.logSave("::::::::::::::::PLCWorker    Start:::::::::::::::: ", 0, "PLC");
            ...
            while (plcRun)
            {
                try
                {
                    ...
                    if (clsCommon.PlantCode == "P102")
                    {
                        lock (plcLock)   // Read 중복을 완전히 차단
                        {
                            Array.Clear(Cimon_Job_Data, 0, Cimon_Job_Data.Length);

                            int result = clsCimonHandler2.Read(
                                0,
                                CIMON_PLC_JOB_ADDR,
                                CIMON_PLC_JOB_READE_SIZE,
                                Cimon_Job_Data
                            );
                        ...
                        // 로깅
                        clsLog.logSave(
                            $"PLC MAP: {Cimon_Job_Data[10]}, {Cimon_Job_Data[11]}, {Cimon_Job_Data[12]}, 50:{Cimon_Job_Data[50]}, 98:{Cimon_Job_Data[98]}",
                            0,
                            "PLC"
                        );
                    }
                    ...
                    // ---------------------------------
                    //   2) UI 업데이트는 600ms마다 1회
                    // ---------------------------------
                    uiCounter++;

                    if (uiCounter >= 3)    // 200ms × 3 = 600ms
                    {
                        uiCounter = 0;

                        if (!isUiBusy && this.IsHandleCreated && !this.IsDisposed)
                        {
                            isUiBusy = true;

                            // ★ tmpDatas를 UI용 복제본으로 전달
                            int[] copy = (int[])tmpDatas.Clone();

                            this.BeginInvoke(new Action(() =>
                            {
                                try
                                {
                                    xDosingResult(copy);   // UI 처리
                                }
                                finally
                                {
                                    isUiBusy = false;
                ...
                {
                    clsLog.logSave("PLCWorker Error: " + ex.Message, 0, "PLC");
                }

                Thread.Sleep(1000);
                clsLog.logSave("::::::::::::::::PLCWorker Pass:::::::::::::::: ", 0, "PLC");
                Console.WriteLine("::::::::::::::::PLCWorker Pass::::::::::::::::");
            }
        ...
        private void xDosingResult(int[] tmpDatas)
        {
            ...
            clsLog.logSave("::::::::::::::::xDosingResult    Start:::::::::::::::: ", 0, "PLC");
                ...
                */
                ...
                for (int p = 0; p < aProcessCode.Length; p++)
                {
                    ...
                    for (int i = 0; i < dtPlc.Rows.Count; i++)
                    {
                        ...
                        //if (!PlcFuncTT.GetPlcCon(dtPlc, out sErrMsg)) return;
                        ...
                        for (int lineNo = 1; lineNo < dtLine.Rows.Count + 1; lineNo++)
                        {
                            ...
                            clsLog.logSave("PLC Read START", 0, "PLC");

                            lock (plcLock)   // Read 중복을 완전히 차단
                            {
                                //PlcFuncTT.PlcGetReadQDeviceBlockEx(dtPlc, vPLC_Location, vPLC_Location == 1 ? clsPlcAddress.WorkInfo : clsPlcAddress.WorkInfo2, ref tmpDatas, "배합 작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);

                                qPlc1.ReadDeviceBlock(clsPlcAddress.WorkInfo, 15, out tmpDatas[0]);
                            }
                            ...
                            lock (plcLock)   // Read 중복을 완전히 차단
                            {
                                //tmpDatas[0] = PlcFuncTT.PlcGetDevice(dtPlc, vPLC_Location, vPLC_Location == 1 ? clsPlcAddress.CurrentBatch : clsPlcAddress.CurrentBatch2, Cimon_Job_Data, BATCH_PV);
                                qPlc1.GetDevice(clsPlcAddress.CurrentBatch, out tmpDatas[0]);
                            }
                            ...
                            // 배치 완료 처리
                            XSetBatchComplete("sPlcType", vProcessKey, vLCode, vPLC_Location);

                            // 작업완료 플래그 처리
                            XSetWorkEnd("sPlcType", vProcessKey, vLCode, vPLC_Location);

                            // 믹서방출시 처리
                            XSetMixerOutput("sPlcType", vProcessKey, vLCode, vPLC_Location);
                        } // for (int lineNo = 1; lineNo < dtLine.Rows.Count + 1; lineNo++)          
            ...
            clsLog.logSave("::::::::::::::::xDosingResult:::::::::::::::: " + sw.Elapsed.TotalSeconds, 0, "PLC");
        }
        ...
        private void XSetBatchComplete(string sPlctype, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                lock (plcLock)   // Read 중복을 완전히 차단
                {
                    //sData[0] = PlcFuncTT.PlcGetDevice(sPlctype, iPlcNo, "D2221", Cimon_Job_Data, BATCH_COMPLETE);

                    qPlc1.GetDevice("D2221", out sData[0]);
                }
                ...
                if (sData[0] == 1 && !vBatchComplete)
                {
                    ...
                    // gubun : 1 배치 2 믹싱
                    PlcSetBatchResult(1, int.Parse(vBatchNum), "Q", iPlcNo, sProcessKey, sLCode);
                    ...
                    //sData[0] = 2;
                    //PlcFuncTT.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, sData, "배치완료 PLC 읽기 실패");
        ...
        /// <returns></returns>
        private void PlcSetBatchResult(int gubun, int batchNum, string sPlcType, int iPlcNo, string sProcessKey, string sLCode)
        {
            DataSet ds;
            DataRow dr;
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                for (int j = 0; j < int.Parse(dr["LOCNT"].ToString()); j++)
                {
                    ...
                    try
                    {
                        ...
                        lock (plcLock)   // Read 중복을 완전히 차단
                        {
                            //valData = PlcFuncTT.PlcGetReadDeviceAddBlock(sPlcType, iPlcType, vPLC_Location == 1 ? clsPlcAddress.Result[Dev] : clsPlcAddress.Result2[Dev], dr, Dev, j, device, number, "스케일 계량값을 가져오지 못했습니다", Cimon_Result_Data, vWorkDate, vWorkNum);

                            qPlc1.ReadDeviceBlock(clsPlcAddress.Result[Dev], 15, out valData[0]);
                        }
                        // GBG -
                    }
                    ...
                    if (Dbconn.conn.getRowCnt(binDs) > 0)
                    {
                        ...
                        if (Dbconn.conn.getRowCnt(remarkDukChkDs) == 0)
                        {
                            ...
                            //    AND a.WORKDATE = '{vWorkDate}' AND a.NUM = '{vWorkNum}' AND BATCH = '{batchNum}'), 'I', '{Dbconn.conn.getData(binDs, "LOCATION", 0)}', '{ScaleVal}', SYSDATE
            ...
            if (vPLCAddress != "")
            {
                lock (plcLock)   // Read 중복을 완전히 차단
                {
                    //PlcFuncTT.PlcGetReadQDeviceBlockEx(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.Ratio : clsPlcAddress.Ratio2, ref rData, "");

                    qPlc1.ReadDeviceBlock(clsPlcAddress.Ratio, 15, out rData[0]);
                }
            }
            else
                rData[0] = 0;
            ...
            //if (clsCommon.PlantCode == "P201")
            //{
            //    // 작지 SAP 실적 생성
            //    if (!clsProcessDosing.SetSAPWorkRemark(clsCommon.PlantCode, sProcessKey, sProcessKey.Merge(sLCode), vWorkDate, vWorkNum, batchNum.ToString()))
            //    {
        ...
        /// </summary>
        /// <param name="tmpDatas"></param>
        private void XSetMixerOutput(string sPlcType, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                vMixComplete = true;
                ...
                lock (plcLock)   // Read 중복을 완전히 차단
                {
                    //tmpDatas[0] = PlcFuncTT.PlcGetDevice(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingComplate : clsPlcAddress.MixingComplate2, Cimon_Job_Data, MIXING_COMPLETE);

                    qPlc1.GetDevice(clsPlcAddress.MixingComplate, out tmpDatas[0]);
                }
                ...
                if (tmpDatas[0] == 1)
                {
                    ...
                    lock (plcLock)   // Read 중복을 완전히 차단
                    {
                        //PlcFuncTT.PlcGetReadQDeviceBlockEx(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingInfo : clsPlcAddress.MixingInfo2, ref tmpDatas, "", Cimon_Job_Data, MIXING_WORK_YYYY);
                        qPlc1.ReadDeviceBlock(clsPlcAddress.MixingInfo, 15, out tmpDatas[0]);
                    }
                    ...
                    lock (plcLock)   // Read 중복을 완전히 차단
                    {
                        //batch = PlcFuncTT.PlcGetDevice(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingCurrentBatch : clsPlcAddress.MixingCurrentBatch2, Cimon_Job_Data, MIXING_BATCH);
                        qPlc1.GetDevice(clsPlcAddress.MixingCurrentBatch, out batch);
                    }
                    ...
                    {
                        // gubun : 1 배치 2 믹싱
                        PlcSetBatchResult(2, batch, sPlcType, iPlcNo, sProcessKey, sLCode);
                    }
                    ...
                    lock (plcLock)   // Read 중복을 완전히 차단
                    {
                        //PlcFuncTT.PlcGetReadQDeviceBlockEx(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.WorkInfo : clsPlcAddress.WorkInfo2, ref tmpDatas, "", Cimon_Job_Data, MIXING_WORK_YYYY);
                        qPlc1.ReadDeviceBlock(clsPlcAddress.WorkInfo, 15, out tmpDatas[0]);
                    }
                    ...
                    lock (plcLock)   // Read 중복을 완전히 차단
                    {
                        //mix_time = PlcFuncTT.PlcGetDevice(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingTime : clsPlcAddress.MixingTime2, Cimon_Job_Data, MIXING_TIME_PV);
                        qPlc1.GetDevice(clsPlcAddress.MixingTime, out mix_time);
                    }
                    ...
                    //PlcFuncTT.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, temp, "믹싱 완료를 실패 했습니다.");
        ...
        /// <returns></returns>
        private string XSetWorkEnd(string sPlcType, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                vWorkComplete = true;
                ...
                lock (plcLock)   // Read 중복을 완전히 차단
                {
                    //rData[0] = PlcFuncTT.PlcGetDevice(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.WorkComplete : clsPlcAddress.WorkComplete2, Cimon_Job_Data, JOB_COMPLETE);
                    qPlc1.GetDevice(clsPlcAddress.WorkComplete, out rData[0]);
                }
                ...
                if (rData[0] == 1)
                {
                    ...
                    lock (plcLock)   // Read 중복을 완전히 차단
                    {
                        PlcFuncTT.PlcGetReadQDeviceBlockEx(sPlcType, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.WorkInfo : clsPlcAddress.WorkInfo2, ref tmpDatas, vWorkDate + "/" + vWorkNum + ":" + vBatchNum + "작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                    }
                    ...
                    tmpDatas[0] = 2;
                    //PlcFuncTT.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, tmpDatas, "작업완료가 실패 했습니다.");
                    if (sPlantCode != "PJ01")
                    {
                        foreach (Form frm in Application.OpenForms)
                        {
