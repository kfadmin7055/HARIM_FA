...
{
    public partial class MAIN_Test : DevExpress.XtraEditors.XtraForm
    {
        /// <summary>        /// 실행 경로y
        /// </summary>
                ...
                case "Q":
                    {
                        qPlc1 = new ActQJ71E71TCP
                        {
                            ...
                            ActSourceStationNumber = int.Parse(dr["N_NO"]?.ToString()) + 3,
        ...
        private void work_watch_timer_Tick(object sender, EventArgs e)
        {
            ...
            {
                xDosingResult2(tmpDatas);
            });

            sw.Stop();

            clsLog.logSave("::::::::::::::::work_watch_timer_Tick:::::::::::::::: " + sw.Elapsed.TotalSeconds, 0, "PLC");
        }

        private void xDosingResult2(int[] tmpDatas)
        {
            try
            {
                Stopwatch sw = new Stopwatch();
                ...
                {
                    int result = qPlc1.ReadDeviceBlock("D2100", 10, out tmpDatas[0]);

                    // PlcFuncTT.PlcGetReadQDeviceBlockEx("Q", 1, "D2100", ref tmpDatas, "배합 작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                }
                ...
                lock (plcLock)   // Read 중복을 완전히 차단
                {
                    int result = qPlc1.GetDevice("D2112", out tmpDatas[0]);
                    //tmpDatas[0] = PlcFuncTT.PlcGetDevice("Q", 1, "D2112", Cimon_Job_Data, BATCH_PV);
                }

                // 배치 완료 처리
                //XSetBatchComplete("Q", vProcessKey, vLCode, 1);

                sw.Stop();

                clsLog.logSave("::::::::::::::::xDosingResult2:::::::::::::::: " + sw.Elapsed.TotalSeconds, 0, "PLC");
            }
            catch (Exception ex)
            {
                MAIN_TT.MainPlcConnChk = "N";
                ...
                */
                ...
                for (int p = 0; p < aProcessCode.Length; p++)
                {
                    ...
                    for (int i = 0; i < dtPlc.Rows.Count; i++)
                    {
                        ...
                        if (!PlcFunc_Test.GetPlcCon(dtPlc, out sErrMsg)) return;
                        ...
                        for (int lineNo = 1; lineNo < dtLine.Rows.Count + 1; lineNo++)
                        {
                            ...
                            PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, vPLC_Location, vPLC_Location == 1 ? clsPlcAddress.WorkInfo : clsPlcAddress.WorkInfo2, ref tmpDatas, "배합 작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                            ...
                            PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, vPLC_Location, vPLC_Location == 1 ? clsPlcAddress.CurrentBatch : clsPlcAddress.CurrentBatch2, ref tmpDatas, "현재 배치 수 읽기 실패");
        ...
        private void XSetBatchComplete(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.BatchComPlate : clsPlcAddress.BatchComPlate2, ref sData, "배치완료 PLC 읽기 실패");
                ...
                if (sData[0] == 1 && !vBatchComplete)
                {
                    ...
                    sData[0] = 2;
                    PlcFunc_Test.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.BatchComPlate : clsPlcAddress.BatchComPlate2, null, 0, sData, "배치완료 PLC 읽기 실패");
        ...
        private void PlcSetBatchResult(int gubun, int batchNum, DataTable dtPlc, int iPlcNo, string sProcessKey, string sLCode)
        {
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                for (int j = 0; j < int.Parse(dr["LOCNT"].ToString()); j++)
                {
                    ...
                    try
                    {
                        ...
                        var (device, number) = (vPLC_Location == 1 ? clsPlcAddress.Result[Dev] : clsPlcAddress.Result2[Dev]).ToString().SplitDeviceAndNumber();
                        ...
                        valData = PlcFunc_Test.PlcGetReadDeviceAddBlock(dtPlc, iPlcType, vPLCAddress, dr, Dev, j, device, number, "스케일 계량값을 가져오지 못했습니다", Cimon_Result_Data, vWorkDate, vWorkNum);
            ...
            if (clsPlcAddress.Ratio != "")
            {
                PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.Ratio : clsPlcAddress.Ratio2, ref rData, "");
            }
            else
                rData[0] = 0;
        ...
        private void XSetMixerOutput(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingComplate : clsPlcAddress.MixingComplate2, ref tmpDatas, "");
                ...
                if (tmpDatas[0] == 1)
                {
                    ...
                    PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingInfo : clsPlcAddress.MixingInfo2, ref tmpDatas, "");
                    ...
                    batch = PlcFunc_Test.PlcGetDevice(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingCurrentBatch : clsPlcAddress.MixingCurrentBatch2, Cimon_Job_Data, MIXING_BATCH);
                    ...
                    PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.WorkInfo : clsPlcAddress.WorkInfo2, ref tmpDatas, "");
                    ...
                    mix_time = PlcFunc_Test.PlcGetDevice(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingTime : clsPlcAddress.MixingTime2, Cimon_Job_Data, MIXING_TIME_PV);
                    ...
                    PlcFunc_Test.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.MixingComplate : clsPlcAddress.MixingComplate2, null, 0, temp, "믹싱 완료를 실패 했습니다.");
        ...
        private string XSetWorkEnd(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                rData[0] = PlcFunc_Test.PlcGetDevice(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.WorkComplete : clsPlcAddress.WorkComplete2, Cimon_Job_Data, JOB_COMPLETE);
                ...
                if (rData[0] == 1)
                {
                    ...
                    PlcFunc_Test.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.WorkInfo : clsPlcAddress.WorkInfo2, ref tmpDatas, vWorkDate + "/" + vWorkNum + ":" + vBatchNum + "작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                    ...
                    tmpDatas[0] = 2;
                    PlcFunc_Test.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLC_Location == 1 ? clsPlcAddress.WorkComplete : clsPlcAddress.WorkComplete2, null, 0, tmpDatas, "작업완료가 실패 했습니다.");
