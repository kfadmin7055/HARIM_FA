...
{
    public partial class MAIN_Test2 : DevExpress.XtraEditors.XtraForm
    {
        /// <summary>        /// 실행 경로y
        /// </summary>

        public static string accText = string.Empty;
        public static string accTag = string.Empty;
        public static string accHint = string.Empty;
        ...
        private void menuClick(object sender, EventArgs e)
        {
            try
            {
                ...
                accText = accControl.Text;
                accTag = accControl.Tag.ToString();
                accHint = accControl.Hint;
                ...
                SetMenuPath(accText, accTag, accHint);
            }
            catch (Exception ex)
            {
        ...
        private void MultiMenuClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dt.Rows[0]["PROCESS_TYPE"]?.ToString() == clsCommon.GetProcessTypeCode("배합") || dt.Rows[0]["PROCESS_TYPE"]?.ToString() == clsCommon.GetProcessTypeCode("갓돈배합"))
                    ...
                    //else
                    //    Form = new frm_Pellet4(dt.Rows[0]["PLANT_CODE"]?.ToString(), dt.Rows[0]["PROCESS_KEY"]?.ToString(), dt.Rows[0]["L_CODE"]?.ToString(), form_name, arrResult[0]);
                else
                {
                    string nameSpace = "HARIM_FA_DOSING";
                    Assembly cuasm = Assembly.GetExecutingAssembly();
                ...
                */
                ...
                for (int p = 0; p < aProcessCode.Length; p++)
                {
                    ...
                    for (int i = 0; i < dtPlc.Rows.Count; i++)
                    {
                        ...
                        if (!PlcFunc_Test2.GetPlcCon(dtPlc, out sErrMsg)) return;
                        ...
                        for (int lineNo = 1; lineNo < dtLine.Rows.Count + 1; lineNo++)
                        {
                            ...
                            PlcFunc_Test2.PlcGetReadQDeviceBlockEx(dtPlc, vPLC_Location, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "배합 작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                            ...
                            PlcFunc_Test2.PlcGetReadQDeviceBlockEx(dtPlc, vPLC_Location, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "현재 배치 수 읽기 실패", Cimon_Job_Data, BATCH_PV);
                            ...
                            if (sPlantCode != "PJ01")
                            {
                                foreach (Form frm in Application.OpenForms)
                                {
                                    ...
                                    //재귀 탐색으로 컨트롤 찾기
                                    System.Windows.Forms.Control txtEdt_runWorkNum = FindControlRecursive(frm, "txtEdt_runWorkNum");
                                    System.Windows.Forms.Control txtEdt_runNum = FindControlRecursive(frm, "txtEdt_runNum");
                                    System.Windows.Forms.Control txtEdt_runWorkBatch = FindControlRecursive(frm, "txtEdt_runWorkBatch");
                                    System.Windows.Forms.Control txtEdt_runWorkProduct = FindControlRecursive(frm, "txtEdt_runWorkProduct");
        ...
        private void XSetBatchComplete(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                PlcFunc_Test2.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref sData, "배치완료 PLC 읽기 실패", Cimon_Job_Data, BATCH_COMPLETE);
                ...
                if (sData[0] == 1 && !vBatchComplete)
                {
                    ...
                    sData[0] = 2;
                    PlcFunc_Test2.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, sData, "배치완료 PLC 읽기 실패");
                    ...
                    if (sPlantCode != "PJ01")
                    {
                        foreach (Form frm in Application.OpenForms)
                        {
                            ...
                            System.Windows.Forms.Control btn = FindControlRecursive(frm, "btn_workSearch");
        ...
        private void PlcSetBatchResult(int gubun, int batchNum, DataTable dtPlc, int iPlcNo, string sProcessKey, string sLCode)
        {
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                for (int j = 0; j < int.Parse(dr["LOCNT"].ToString()); j++)
                {
                    ...
                    try
                    {
                        ...
                        valData = PlcFunc_Test2.PlcGetReadDeviceAddBlock(dtPlc, iPlcType, vPLCAddress, vPLCUnit, vPLCDataCount, dr, Dev, j, device, number, "스케일 계량값을 가져오지 못했습니다", Cimon_Result_Data, vWorkDate, vWorkNum);
            ...
            if (vPLCAddress != "")
            {
                PlcFunc_Test2.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref rData, "");
            }
            else
                rData[0] = 0;
        ...
        private void XSetMixerOutput(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                PlcFunc_Test2.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "", Cimon_Job_Data, MIXING_COMPLETE);
                ...
                if (tmpDatas[0] == 1)
                {
                    ...
                    PlcFunc_Test2.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "", Cimon_Job_Data, MIXING_WORK_YYYY);
                    ...
                    batch = PlcFunc_Test2.PlcGetDevice(dtPlc, iPlcNo, vPLCAddress, vPLCDataCount, Cimon_Job_Data, MIXING_BATCH);
                    ...
                    mix_time = PlcFunc_Test2.PlcGetDevice(dtPlc, iPlcNo, vPLCAddress, vPLCDataCount, Cimon_Job_Data, MIXING_TIME_PV);
                    ...
                    PlcFunc_Test2.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, temp, "믹싱 완료를 실패 했습니다.");
        ...
        private string XSetWorkEnd(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                rData[0] = PlcFunc_Test2.PlcGetDevice(dtPlc, iPlcNo, vPLCAddress, vPLCDataCount, Cimon_Job_Data, JOB_COMPLETE);
                ...
                if (rData[0] == 1)
                {
                    ...
                    PlcFunc_Test2.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, vWorkDate + "/" + vWorkNum + ":" + vBatchNum + "작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                    ...
                    PlcFunc_Test2.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, tmpDatas, "작업완료가 실패 했습니다.");
                    ...
                    if (clsCommon.GetAutoTrans() == "Y" || clsCommon.GetTransAuto(sPlantCode, sProcessKey) == "Y")
                    {
                        ...
                        WHERE PLANT_CODE = '{clsCommon.PlantCode}' AND PROCESS_KEY = '{sProcessKey}' AND L_CODE = '{sProcessKey.Merge(sLCode)}'
                                ...
                                AND DEL_FLAG != 'Y' AND RESOURCE_NO NOT LIKE 'F%' AND NVL(TRANS_YN, 'N') != 'Y'
                        ";
        ...
        private static void SetDosingWorkInfo(string vWorkDate, string vWorkNum, string vResourceName, string vBatchNum)
        {
            foreach (Form f in Application.OpenForms)
            {
                ...
                {
                    System.Windows.Forms.Control[] ctrls = f.Controls.Find("txtEdt_runWorkNum", true);
                    if (ctrls.Length > 0)
                    {
        ...
        System.Windows.Forms.Control FindControlRecursive(System.Windows.Forms.Control parent, string controlName)
        {
            foreach (System.Windows.Forms.Control ctrl in parent.Controls)
            {
                ...
                System.Windows.Forms.Control found = FindControlRecursive(ctrl, controlName);
                if (found != null)
                    return found;
            }
            return null;
        ...
        private void xtraTabbedMdiManager_SelectedPageChanged(object sender, EventArgs e)
        {
            ...
            {
                // 비활성 탭
                page.Appearance.Header.BackColor = Color.LightGray;
            ...
            {
                manager.SelectedPage.Appearance.Header.BackColor = Color.DodgerBlue;
                manager.SelectedPage.Appearance.Header.ForeColor = Color.White;

                SetMenuPath(manager.SelectedPage.Text, manager.MdiParent.ActiveMdiChild.Name, "");
            }
        }
    }
}