    ...
    public partial class MAIN : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void MultiMenuClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (arrResult[0] != "" && dt != null && dt.Rows.Count == 0)
                {
                ...
                {
                    sPlantCode = dt.Rows[0]["PLANT_CODE"]?.ToString();
                    sProcessType = dt.Rows[0]["PROCESS_TYPE"]?.ToString();
                    sProcessKey = dt.Rows[0]["PROCESS_KEY"]?.ToString();
                    sLcode = dt.Rows[0]["L_CODE"]?.ToString();
                }

                if (form_name.ToUpper().Contains("BIN"))
                    Form = new frm_Bin(sPlantCode, sProcessKey);
                else if (sProcessType == clsCommon.GetProcessTypeCode("배합") || sProcessType == clsCommon.GetProcessTypeCode("갓돈배합"))
                    Form = new frm_Dosing(sPlantCode, sProcessKey, sLcode, form_name, arrResult[0]);
                else if (sProcessType == clsCommon.GetProcessTypeCode("포장"))
                    Form = new frm_Pack(sPlantCode, sProcessKey, sLcode, form_name, arrResult[0]);
                else if (sProcessType == clsCommon.GetProcessTypeCode("펠렛"))
                    //if (Debugger.IsAttached)
                    Form = new frm_Pellet(sPlantCode, sProcessKey, sLcode, form_name, arrResult[0]);
                //else
        ...
        private void conn_witch_timer_Tick(object sender, EventArgs e)
        {
            ...
            if (Properties.Settings.Default.MainPlc_Yn == "Y" && MainPlcConnChk != "Y" && !Debugger.IsAttached)
            {
            ...
            if (Properties.Settings.Default.SubPlc_Yn == "Y" && SubPlcConnChk != "Y" && !Debugger.IsAttached)
            {
                ...
                case "Q":
                    {
                        qPlc1 = new ActQJ71E71TCP
                        {
                            ...
                            ActTimeOut = Convert.ToInt32(dr["T_OUT"] == DBNull.Value ? 0 : dr["T_OUT"])
                        };
        ...
        private void PlcSetBatchResult(int gubun, int batchNum, DataTable dtPlc, int iPlcNo, string sProcessKey, string sLCode)
        {
            ...
            UPDATE WORK_ORDER
                SET R_BATCH = CASE
                    WHEN '{batchNum}' = BATCH
                    THEN TO_CHAR(TO_NUMBER('{batchNum}'))

                    WHEN PLANT_CODE IN ('PJ01', 'PJ02', 'PJ04', 'PJ05')
                    THEN TO_CHAR(TO_NUMBER('{batchNum}') + 1)

                    ELSE TO_CHAR(TO_NUMBER('{batchNum}'))
                END,
        ...
        private string XSetWorkEnd(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                if (rData[0] == 1)
                {
                    ...
                    if (clsCommon.GetAutoTrans() == "Y" || clsCommon.GetTransAuto(clsCommon.PlantCode, sProcessKey) == "Y")
                    {
