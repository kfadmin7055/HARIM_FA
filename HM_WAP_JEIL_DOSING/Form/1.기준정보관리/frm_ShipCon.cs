...
{
    public partial class frm_ShipCon : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void frm_ShipCon_Load(object sender, EventArgs e)
        {
        ...
        private void XMain_Search()
        {
            ...
            {
                SQL = $@"
                SELECT SHIP_NAME, TO_CHAR(I_TIME, 'YYYY-MM-DD HH24:MI:SS') AS I_TIME /* 01 */
                FROM SHIP_CON a
                ";
        ...
        private void XMain_Save()
        {
            try
            {
                ...
                foreach (DataRow dr in DT.Rows)
                {
                    ...
                    {
                        SQL = $@"
                        INSERT INTO SHIP_CON(SHIP_NAME
                                            , I_TIME)
                        VALUES ( '{dr["SHIP_NAME"]}'   /* 01 */
                               , SYSDATE               /* 02 I_TIME */
                        )";
                    ...
                    else if (dr.RowState == DataRowState.Modified)
                    {
                        SQL = $@"
                        UPDATE SHIP_CON
                           SET SHIP_NAME = '{dr["SHIP_NAME"]}'
                             , I_TIME = SYSDATE
                         WHERE SHIP_NAME = '{dr["SHIP_NAME"]}'
                        ";
        ...
        private void XMain_Delete()
        {
            try
            {
                ...
                SQL = $@"
                DELETE FROM SHIP_CON
                    WHERE SHIP_NAME = '{gridView.GetFocusedRowCellValue("SHIP_NAME")}'
                ";
