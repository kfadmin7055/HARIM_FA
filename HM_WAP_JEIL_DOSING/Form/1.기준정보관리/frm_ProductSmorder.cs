...
{
    public partial class frm_ProductSmorder : DevExpress.XtraEditors.XtraForm
    {
        private string SQL = String.Empty;
        DataSet authDs;
        private string[] sValid = null;

        public frm_ProductSmorder()
        {
            InitializeComponent();
            clsDevexpressGrid.EditGridViewInit(gridView, Properties.Settings.Default.FontSize);
        }

        private void frm_ProductSmorder_Load(object sender, EventArgs e)
        {
            ...
            clsDevexpressGrid.ItemSearchLookUpEditSetup(gridScboPartner, clsCommon.GetCustomer(), "", false);
        ...
        private void XMain_Search()
        {
            try
            {
                ...
                , a.EMPLOYEE_NO
                , a.I_TIME
                FROM PRODUCT_SMORDER a
                    JOIN SAP_DI_CUSTOMER b ON b.PARTNER = a.PARTNER
                WHERE (a.PARTNER LIKE '%{txtCustomer.EditValue}%' OR b.NAME_ORG1 LIKE '%{txtCustomer.EditValue}%')
                ORDER BY a.PLANT_CODE, a.PARTNER
                ";
        ...
        private void XMain_Save()
        {
            try
            {
                ...
                foreach (DataRow dr in DT.Rows)
                {
                    ...
                    {
                        SQL = $@"
                        INSERT INTO PRODUCT_SMORDER (
                           EMPLOYEE_NO, I_TIME, PARTNER, 
                           PLANT_CODE, RESOURCE_NO, VEHICLENO) 
                        VALUES ( '{dr["EMPLOYEE_NO"]}',
                         SYSDATE,
                         '{dr["PARTNER"]}',
                         '{dr["PLANT_CODE"]}',
                         '{dr["RESOURCE_NO"]}',
        ...
        private void XMain_Delete()
        {
            try
            {
                ...
                SQL = $@"
                DELETE FROM PRODUCT_SMORDER 
                WHERE   PLANT_CODE    = '{gridView.GetFocusedRowCellValue("PLANT_CODE")}'
                    AND PARTNER       = '{gridView.GetFocusedRowCellValue("PARTNER")}'
                    AND VEHICLENO     = '{gridView.GetFocusedRowCellValue("VEHICLENO")}'
                ";
