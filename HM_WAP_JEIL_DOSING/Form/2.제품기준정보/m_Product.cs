...
namespace HM_WAP_JEIL_DOSING
{
    public partial class m_Product : DevExpress.XtraEditors.XtraForm
    {
        ...
        public m_Product(string sPlantCode)
        {
            vPlantCode = sPlantCode;
            InitializeComponent();
            clsDevexpressGrid.ReadGridViewInit(viewProduct);
        }
        ...
        private void XMain_Search()
        {
            try
            {
                ...
                FROM SAP_DI_PRODUCT a
                    LEFT JOIN SAP_IN_PRODUCT_CH b ON b.RESOURCE_NO = a.RESOURCE_NO
                    LEFT JOIN SAP_IN_PRODUCT_RC c ON c.RESOURCE_NO = a.RESOURCE_NO
                WHERE a.PLANT_CODE = '{vPlantCode}'
                    AND ('{txtRESOURCE.EditValue}' IS NULL OR a.DESCRIPTION LIKE '%{txtRESOURCE.EditValue}%')
                    AND a.SU_CODE IS NULL
                    {suGubun}
                ...
                // 저장위치
                clsDevexpressGrid.ItemSearchLookUpEditSetup(gridscboHOME_LOCATION, clsCommon.GetLocation());
        ...
        private void XPartSearch(string vResourceNo)
        {
            try
            {
                SQL = $@"
        ...
        private void XResSearch(string vResourceNo)
        {
            try
            {
                SQL = $@"
        ...
        private void XCPSearch(string vResourceNo)
        {
            try
            {
                SQL = $@"
        ...
        {
            if (e.FocusedRowHandle >= 0 && e.FocusedRowHandle < viewProduct.RowCount)
            {
                try
                {
                    vResourceNo = clsDevexpressGrid.GetFocusedRowCellValue(viewProduct, "RESOURCE_NO");

                    XPartSearch(vResourceNo);
                    XResSearch(vResourceNo);
                    XCPSearch(vResourceNo);
                }
                catch (Exception ex)
                {
