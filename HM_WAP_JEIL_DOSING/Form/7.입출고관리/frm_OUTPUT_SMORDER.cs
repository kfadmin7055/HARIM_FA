...
{
    public partial class frm_OUTPUT_SMORDER : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void frm_SCR_MG_Load(object sender, EventArgs e)
        {
            ...
            clsDevexpressGrid.LoadGridColumnSeq(this.Name, gridMain, viewDetail);

            viewDecar.OptionsBehavior.EditorShowMode = DevExpress.Utils.EditorShowMode.MouseDown;
            clsDevexpressGrid.LoadGridColumnSeq(this.Name, gridDecar, viewDecar);

            authDs = clsSql.GetAuthDataSet(this.Name);

            //배송일자
            dtFromDeliveryDate.EditValue = DateTime.Today.ToString("yyyy-MM-dd");
            dtToDeliveryDate.EditValue = DateTime.Today.AddDays(1).ToString("yyyy-MM-dd");
        ...
        private void InitControl()
        {
            ...
            clsDevexpressGrid.ItemLookUpEditSetup(gridcboTransFlag1, clsCommon.GetTransFlag());

            // 차량타입
            clsDevexpressGrid.ItemLookUpEditSetup(gridcboCAR_TYPE, clsCommon.GetCarInputType(), "", false, false);
            clsDevexpressGrid.ItemLookUpEditSetup(gridcboCAR_TYPE1, clsCommon.GetCarInputType(), "", false, false);

            // 차량번호
            clsDevexpressGrid.ItemLookUpEditSetup(gridCboVEHICLEGROUPCODE, clsCommon.getCarGroupType(), "", false, false);
            clsDevexpressGrid.ItemLookUpEditSetup(gridCboVEHICLEGROUPCODE1, clsCommon.getCarGroupType(), "", false, false);

            // 진행상태
            clsDevexpressGrid.ItemLookUpEditSetup(gridcboPC_STATUS, clsCommon.GetCarStatus(), "", false, false);
            clsDevexpressGrid.ItemLookUpEditSetup(gridcboPC_STATUS1, clsCommon.GetCarStatus(), "", false, false);
            ...
            clsDevexpressGrid.ItemSearchLookUpEditSetup(gridscboINCAR_NO1, clsCommon.GetCarMaster(), "", true, parameterDict);
        }
        ...
        private void XMain_Search()
        {
            try
            {
                ...
                SELECT DISTINCT
                   ...
                   , NVL(b.ZERO_W, 0) AS OUT_WEIGHT         -- 출차중량
                ...
                clsDevexpressGrid.ItemLookUpEditSetup(gridMcboUNIT, clsCommon.GetUnit(), "", false, false);
        ...
        private void btn_rowAdd_Click(object sender, EventArgs e)
        {
            ...
            viewDecar.SetRowCellValue(newRowHandle, viewDecar.Columns["IS_NO"], "SM".Merge(DateTime.Now.ToString("yyyyMMddHHmmss")));
            viewDecar.SetRowCellValue(newRowHandle, viewDecar.Columns["WEIGHT_KG"], DateTime.Now.ToString("yyyyMMddHHmmss"));
            viewDecar.SetFocusedRowCellValue("CAR_TYPE", clsCommon.GetCarInputTypeCode("홍성물류"));
            viewDecar.SetFocusedRowCellValue("TEM_TYPE", "N");
            viewDecar.SetFocusedRowCellValue("DEL_FLAG", "N");
            viewDecar.SetFocusedRowCellValue("PRINT_YN", "N");
            viewDecar.SetFocusedRowCellValue("ERP_UP_YN", "N");
        ...
        private void btn_rowAdd1_Click(object sender, EventArgs e)
        {
            ...
            viewDetail.SetFocusedRowCellValue("BSTKD", $"SM{viewDecar.GetFocusedRowCellValue("IS_NO")}");
            viewDetail.SetFocusedRowCellValue("IS_NO", viewDecar.GetFocusedRowCellValue("IS_NO"));

            viewDetail.SetFocusedRowCellValue("HEADTEXT", viewDecar.GetFocusedRowCellDisplayText("INCAR_NO"));
                ...
                foreach (DataRow dr in DT.Rows)
                {
                    ...
                    if (dr.RowState == DataRowState.Modified)
                    {
                        ...
                        SET
                                 ...
                                 , VRKME       = '{dr["VRKME"]}'                -- 4
                                 , AUART       = '{dr["AUART"]}'
                                 , KUNNR_SP    = '{dr["KUNNR_SP"]}'
                                 , KUNNR_SH    = '{dr["KUNNR_SH"]}'
                                 , ERP_UP_YN   = CASE TO_CHAR(ERP_UP_YN) -- WHEN 'Y' THEN 'M' 
                                                            WHEN 'U' THEN 'M'
                                                                WHEN 'D' THEN 'X'
                                                                WHEN 'F' THEN 'N'
        ...
        private void btn_delete1_Click(object sender, EventArgs e)
        {
            ...
            if (viewDetail.SelectedRowsCount == 0)
            {
                XtraMessageBox.Show("삭제하실 데이터를 선택하여 주세요");
                return;
            }
        ...
        private void XDetail_Delete()
        {
            try
            {
                ...
                SQL = $@"
                DELETE FROM SAP_OUTPUT_SMORDER
                WHERE IS_NO = '{viewDetail.GetFocusedRowCellValue("IS_NO")}'
                    AND POSNR = '{viewDetail.GetFocusedRowCellValue("POSNR")}'
                ";
                ...
                XtraMessageBox.Show("삭제 완료 되었습니다.");
        ...
        {
            gridDecar.Focus();
            viewDecar.FocusedRowHandle = 0;
            viewDecar.FocusedColumn = viewDecar.VisibleColumns[0];
        }
        ...
        private void PrintReport()
        {
            ...
            try
            {
                ...
                if (tabPage.SelectedTabPageIndex == 0)
                {
                    sIsNo = viewMain.GetFocusedRowCellValue("IS_NO")?.ToString();
                    posnr = viewMain.GetFocusedRowCellValue("POSNR")?.ToString();
                }
                else
                    sIsNo = viewDecar.GetFocusedRowCellValue("IS_NO")?.ToString();
                ...
                clsPrintReport.PrinWeighingReport_HS(sIsNo, posnr);
            }
            catch (Exception ex)
            {
