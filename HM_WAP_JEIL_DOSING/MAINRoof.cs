...
{
    public partial class MAINRoof : DevExpress.XtraEditors.XtraForm
    {
        /// <summary>        /// 실행 경로y
        /// </summary>
                ...
                */
                ...
                for (int p = 0; p < aProcessCode.Length; p++)
                {
                    ...
                    for (int i = 0; i < dtPlc.Rows.Count; i++)
                    {
                        ...
                        if (!PlcFuncRoof.GetPlcCon(dtPlc, out sErrMsg)) return;
                        ...
                        for (int lineNo = 1; lineNo < dtLine.Rows.Count + 1; lineNo++)
                        {
                            ...
                            PlcFuncRoof.PlcGetReadQDeviceBlockEx(dtPlc, vPLC_Location, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "배합 작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                            ...
                            PlcFuncRoof.PlcGetReadQDeviceBlockEx(dtPlc, vPLC_Location, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "현재 배치 수 읽기 실패", Cimon_Job_Data, BATCH_PV);
        ...
        private void XSetBatchComplete(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                PlcFuncRoof.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref sData, "배치완료 PLC 읽기 실패", Cimon_Job_Data, BATCH_COMPLETE);
                ...
                if (sData[0] == 1 && !vBatchComplete)
                {
                    ...
                    sData[0] = 2;
                    // 디버그
                    //PlcFuncRoof.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, sData, "배치완료 PLC 읽기 실패");
        ...
        private void PlcSetBatchResult(int gubun, int batchNum, DataTable dtPlc, int iPlcNo, string sProcessKey, string sLCode)
        {
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                for (int j = 0; j < int.Parse(dr["LOCNT"].ToString()); j++)
                {
                    ...
                    try
                    {
                        ...
                        valData = PlcFuncRoof.PlcGetReadDeviceAddBlock(dtPlc, iPlcType, vPLCAddress, vPLCUnit, vPLCDataCount, dr, Dev, j, device, number, "스케일 계량값을 가져오지 못했습니다", Cimon_Result_Data, vWorkDate, vWorkNum);
            ...
            if (vPLCAddress != "")
            {
                PlcFuncRoof.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref rData, "");
            }
            else
                rData[0] = 0;
        ...
        private void XSetMixerOutput(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                PlcFuncRoof.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "", Cimon_Job_Data, MIXING_COMPLETE);
                ...
                if (tmpDatas[0] == 1)
                {
                    ...
                    PlcFuncRoof.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, "", Cimon_Job_Data, MIXING_WORK_YYYY);
                    ...
                    batch = PlcFuncRoof.PlcGetDevice(dtPlc, iPlcNo, vPLCAddress, vPLCDataCount, Cimon_Job_Data, MIXING_BATCH);
                    ...
                    mix_time = PlcFuncRoof.PlcGetDevice(dtPlc, iPlcNo, vPLCAddress, vPLCDataCount, Cimon_Job_Data, MIXING_TIME_PV);
                    ...
                    //// 디버그
                    //PlcFuncRoof.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, temp, "믹싱 완료를 실패 했습니다.");
        ...
        private string XSetWorkEnd(DataTable dtPlc, string sProcessKey, string sLCode, int iPlcNo)
        {
            try
            {
                ...
                rData[0] = PlcFuncRoof.PlcGetDevice(dtPlc, iPlcNo, vPLCAddress, vPLCDataCount, Cimon_Job_Data, JOB_COMPLETE);
                ...
                if (rData[0] == 1)
                {
                    ...
                    PlcFuncRoof.PlcGetReadQDeviceBlockEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, ref tmpDatas, vWorkDate + "/" + vWorkNum + ":" + vBatchNum + "작지 PLC 읽기 실패", Cimon_Job_Data, WORK_YYYY);
                    ...
                    tmpDatas[0] = 2;
                    //// 디버그
                    //PlcFuncRoof.PlcSetQDeviceEx(dtPlc, iPlcNo, vPLCAddress, vPLCUnit, vPLCDataCount, null, 0, tmpDatas, "작업완료가 실패 했습니다.");
                    if (sPlantCode != "PJ01")
                    {
                        foreach (Form frm in Application.OpenForms)
                        {
